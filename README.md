...
### Getting started
In order to start the game we need to launch the server. On a terminal, go to the root folder, than run ```cd server```, followed by the command ```java -jar server.jar```. You can do it on either a machine connected to your LAN or on the same computer you will play on. You will see the sentence "Server listening" when everything will be ready.

**Guide for *cli* users.** Each player that wants to play with the command-line has to launch the client on a different terminal (or machine connected in the same LAN). On a terminal, go to the root folder, than use the command ```cd client/cli``` followed by the command ```java -jar cli.jar```. 

**Guide for *gui* users.**  Each player that wants to play with the graphic interface has to launch the client on a different terminal (or machine connected in the same LAN). On a terminal, go to the root folder, than use the command ```cd client/gui``` followed by the command ```java -jar gui.jar```. 
